/* ══ GitHub Storage — works in every browser ══
   Saves portfolio-data.json directly to GitHub via API.
   Netlify auto-redeploys when the file changes (~30 seconds).
   ══════════════════════════════════════════════════════ */
...
async function save(){
  ...
  if(!ok){
    toast("⚠️ Save to GitHub failed. Check your internet connection and try again.","error");
  }
}

// After saving to GitHub, Netlify redeploys in ~30s.
// portfolio-data.json on the live site updates automatically.
