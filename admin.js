...
async function save(){
  ...
  if(!ok) console.warn("Saved to localStorage only.");
}
...
function showSaved(){
  const b=document.getElementById("saveBadge");
  b.textContent="Saved ✓";b.style.color="var(--success)";b.classList.add("visible");
...
function processPostImage(file){
  ...
  reader.onload=e=>{
    ...
    img.onload=()=>{
      ...
      const c=document.createElement("canvas");c.width=w;c.height=h;
      c.getContext("2d").drawImage(img,0,0,w,h);
      _pendingPostImage=c.toDataURL("image/jpeg",.82);
...
async function saveAll(){
  ...
  await save();showSaved();toast("All changes saved ✓");
}
