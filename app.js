/* ══════════════════════════════════════════════════════════════
   STORAGE — Reads via Netlify Function (server-side)
   ────────────────────────────────────────────────────────────
   app.js calls /.netlify/functions/get-data
   The function reads portfolio-data.json from GitHub on the SERVER.

   This means the browser only ever talks to YOUR OWN Netlify domain.
   Instagram, WhatsApp, Chrome, Safari — every browser works. ✅
   No API keys in the browser. No CORS issues. No caching problems.
   ══════════════════════════════════════════════════════════════ */

async function loadFromFirebase(defaultData) {
  // Primary: Netlify function (server-side GitHub read — works in ALL browsers)
  try {
  ...
  // Fallback: direct file (works when running locally / Netlify function not deployed)
  try {
